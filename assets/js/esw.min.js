...
(function (l) {
    function d() {
        ...
        this.storageKeys =
            ["ESW_BODY_SCROLL_POSITION", "ESW_IS_MINIMIZED", "ESW_MINIMIZED_TEXT", "ESW_OAUTH_TOKEN"];
    ...
    d.prototype.loadLightningOutScripts = function (a) {
        if ("function" !== typeof Promise)
            this.loadScriptFromDirectory("common", "promisepolyfill", function () {
    ...
    d.prototype.validateStartChatAttributes = function (a) {
        ...
        if (Array.isArray(f))
            ...
            throw Error("Validation failed for fallbackRouting, received: " +
                f);
        ...
        if ("object" === typeof g)
            if (a.directToAgentRouting = g, g.buttonId)
                if ("string" === typeof g.buttonId && g.buttonId.trim().length)
                    a.directToAgentRouting.buttonId = g.buttonId.trim(), a.directToAgentRouting.fallback = g.fallback, g.userId && ("string" === typeof g.userId && g.userId.trim().length ? a.directToAgentRouting.userId = g.userId.trim() : embedded_svc.log("Did not receive an actionable parameter for directToAgentRouting's userId, received: " +
                                g.userId));
                else
    ...
    d.prototype.outputToConsole = function (a, b, c) {
        if ((c || this.settings.devMode) && console && console[a])
            console[a]("[Snap-ins] " +
    ...
    d.prototype.getCookie = function (a) {
        ...
        if (b) {
            var c = b.indexOf(a + "=");
            if (-1 !== c)
                return c += (a + "=").length, a = b.indexOf(";", c), -1 === a && (a = b.length), b.substring(c, a)
        }
    };
    ...
    d.prototype.loadFeatureScript = function (a, b) {
        var c = decodeURI(a).toLowerCase();
        -1 === a.indexOf("..") ? this.loadScriptFromDirectory("client", c + ".esw", function () {
    ...
    d.prototype.fireEvent = function (a, b) {
        ...
        if (window.embedded_svc && embedded_svc.utils)
            return embedded_svc.utils.fireEvent(a, b, c);
    ...
    d.prototype.appendIFrame = function () {
        ...
        window.addEventListener("beforeunload", function (c) {
            ...
            if (this.warnOnBeforeUnload && this.componentLoaded)
                if (embedded_svc.utils.fireEvent("snapinsCloseSessionWarning"), this.settings.closeSessionWarning && "function" === typeof this.settings.closeSessionWarning)
                    this.settings.closeSessionWarning();
                else
                    return (c || window.event).returnValue = "You might lose the active chat session if you close this tab. Are you sure?"
        }
            .bind(this), !1)
    };
    ...
    d.prototype.loadLightningApp = function (a) {
        var b;
        if (this.settings.isExternalPage && "string" !== typeof this.settings.communityEndpointURL)
            throw Error("communityEndpointURL String property not set");
    ...
    d.prototype.setHelpButtonText = function (a, b) {
        ...
        if (c) {
            ...
            e.innerHTML = a;
            if (a = c.parentElement.querySelector(".embeddedServiceIcon"))
    ...
    d.prototype.init = function (a, b, c, e, f, h) {
        ...
        if ("string" !== typeof this.settings.baseCoreURL)
            throw Error("Base core URL value must be a string.");
        if (!this.isOrganizationId(this.settings.orgId))
            throw Error("Invalid OrganizationId Parameter Value: " + this.settings.orgId);
    ...
    d.prototype.onHelpButtonClick = function () {
        if (!this.componentInitInProgress && !document.getElementsByClassName("embeddedServiceSidebar").length) {
            ...
            try {
                ...
                throw this.componentInitInProgress = !1,
                a;
            }
        }
    };
    ...
    d.prototype.setupMessageListener = function () {
        window.addEventListener("message", function (a) {
            ...
            if (b && b.method && embedded_svc.isMessageFromSalesforceDomain(c) &&
                ...
                if (c !== f)
                    if ("session.frameReady" === b.method)
                        this.settings.iframeURL = this.settings.iframeURL.replace(f, c);
                    else
                        return;
                a = b.method.split(".")[0].toLowerCase();
    ...
    /*d.prototype.isMessageFromSalesforceDomain = function (a) {
        if (embedded_svc.utils.isCommunityOrSite() && a === document.domain)
            return !0;
        var b = function (c, e) {
    ...
    d.prototype.requireAuthentication = function () {
        ...
        if ("https:" !== window.location.protocol && !this.settings.devMode)
            throw this.settings.displayHelpButton = !1, Error("Snap-in authentication requires HTTPS.");
        if (!this.settings.useCustomAuthentication) {
            if (!this.settings.loginClientId ||
                !this.settings.loginRedirectURL || !this.settings.loginTargetQuerySelector)
            ...
            if (c)
                this.loginButtonPressed = !1, c.addEventListener("click", function () {
                    this.loginButtonPressed = !0
                }
                    .bind(this));
            else
                throw Error("loginTargetQuerySelector is not a valid DOM element.");
